...
export default function ThreeScene() {
  ...
  useEffect(() => {
    const width = 650;  // Set to 650px as per your requirement
    const height = 650; 
    ...
    const ribbons = [];
    const ribbonCount = 10;  // Number of ribbons for shadow effect
    for (let i = 0; i < ribbonCount; i++) {
      const ribbon = createSRibbon();
      ribbon.position.z = -i * 0.1;  // Slight offset in z-axis for shadow effect
      ribbons.push(ribbon);
      scene.add(ribbon);
    }
    ...
    camera.position.set(0, 0, 14);  // Move camera closer for a larger appearance
    ...
    // Bloom pass setup with higher initial strength for stronger glow effect
    ...
    const animate = function () {
      ...
      if (zoomingIn) {
        ...
        bloomPass.strength = Math.min(bloomPass.strength + 0.05, 2.0);  // Stronger glow as it zooms in
        if (camera.position.z <= 10) {  // Adjust zooming limits to match new camera position
          zoomingIn = false;  // Start zooming out when close enough
        }
      } else {
        camera.position.z += 0.05;
        bloomPass.strength = Math.max(bloomPass.strength - 0.05, 0.0);  // Weaker glow as it zooms out
        if (camera.position.z >= 14) {  // Adjust zooming limits to match new camera position
          zoomingIn = true;  // Start zooming in again when back to original position
        }
      }
    ...
    const handleResize = () => {
      ...
      renderer.setSize(width, height);
      composer.setSize(width, height);  // Update composer on resize
